...
export function SubscriptionPage({ session }: { session: AuthSession }) {
  ...
  useEffect(() => {
    apiRequest<Subscription>("/api/v1/subscription", {}, session)
      ...
      .catch((e) =>
        setError(
          e instanceof Error ? e.message : "No pudimos consultar tu plan.",
        ),
      );
  }, [session]);
  async function requestChange(planCode: string) {
    setBusy(planCode);
    setError("");
    setMessage("");
    try {
  ...
  const plan = plans[data.planCode] ?? plans.negocio,
    end =
      data.subscriptionStatus === "Trial"
        ? data.trialEndsAtUtc
        : data.currentPeriodEndsAtUtc,
    days = end
  ...
  return (
    <div className="content subscription-page">
      <section className="subscription-heading">
        <div>
          <p className="eyebrow">CUENTA Y FACTURACIÓN</p>
          <h1>Mi plan</h1>
          <p>Consulta tu prueba, vigencia y pagos registrados.</p>
        </div>
      </section>
      ...
      <section className="subscription-hero">
        <div>
          <span className="subscription-status">
            <ShieldCheck />
            {labels[data.subscriptionStatus] ?? data.subscriptionStatus}
          </span>
        ...
        <aside>
          ...
          <strong>
            {end
              ? new Date(end).toLocaleDateString("es-MX", {
                  day: "numeric",
                  month: "long",
                  year: "numeric",
                })
          ...
          <small>
            {data.graceEndsAtUtc && (
              <>
                Acceso de gracia hasta el{" "}
                {new Date(data.graceEndsAtUtc).toLocaleDateString("es-MX")}.
              </>
            )}
          </small>
        </aside>
      ...
      <section className="plan-options">
        ...
        <div>
          {Object.entries(plans).map(([code, item]) => (
            <article
              className={code === data.planCode ? "current" : ""}
              key={code}
            >
              <h3>{item.name}</h3>
              <strong>
              ...
              <p>
                {code === data.planCode
                  ? "Tu plan actual"
                  : "Solicita el cambio sin perder tu información."}
              </p>
              <ul className="plan-comparison-features">
                {item.features.map((feature) => <li key={feature}><Check />{feature}</li>)}
              </ul>
              <button
                disabled={code === data.planCode || !!busy}
                onClick={() => void requestChange(code)}
              >
                {code === data.planCode
                  ? "Plan actual"
                  : busy === code
      ...
      <section className="payment-history">
        <div>
          ...
          <span>
            ...
            <p>Pagos registrados por nuestro equipo.</p>
          </span>
        </div>
        {data.payments.length ? (
          <table>
            ...
            <tbody>
              {data.payments.map((x) => (
                <tr key={x.id}>
                  <td>{new Date(x.paidAtUtc).toLocaleDateString("es-MX")}</td>
                  <td>
                    {new Date(x.periodStartsAtUtc).toLocaleDateString("es-MX")}{" "}
                    – {new Date(x.periodEndsAtUtc).toLocaleDateString("es-MX")}
                  </td>
                  <td>{x.method}</td>
                  <td>{x.reference ?? "—"}</td>
                  <td>
          ...
          <div className="empty-payments">
            <CalendarDays />
            <p>
              Todavía no hay pagos registrados. Tu periodo de prueba no requiere
              tarjeta.
            </p>
          </div>
        )}
